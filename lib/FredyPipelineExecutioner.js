...
import { getPolygonFeatures, isListingInSpatialFilter } from './services/spatial.js';
...
class FredyPipelineExecutioner {
  ...
  _filterByArea(newListings) {
    const polygonFeatures = getPolygonFeatures(this._jobSpatialFilter);
    ...
    // Keep only listings within the polygon (fail-open for listings without
    // coordinates). Decision comes from the shared spatial predicate so the crawl
    // and the read path (queryListings) stay in lockstep.
    const keptListings = newListings.filter((listing) => {
      const keep = isListingInSpatialFilter(listing, this._jobSpatialFilter, { failOpenNoCoord: true });
      if (!keep) {
