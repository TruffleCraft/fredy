...
import { getPolygonFeatures, isListingInSpatialFilter } from '../spatial.js';
 ...
 * @param {import('../../types/filter.js').SpatialFilter|null} [params.spatialFilter=null] - Optional job area filter.
...
export const queryListings = ({
  ...
  // SQLite cannot evaluate GeoJSON polygons.  Keep the regular SQL filters and
  // sorting as the first stage, then apply the same point-in-polygon predicate
  // used by the crawl pipeline before paginating.  Listings without a location
  // remain visible and are marked in the MCP response as not yet verifiable.
  if (getPolygonFeatures(spatialFilter).length > 0) {
    const spatialParams = { ...params };
    delete spatialParams.limit;
    delete spatialParams.offset;
    const filteredRows = SqliteConnection.query(selectRowsSql, spatialParams).filter((listing) =>
      isListingInSpatialFilter(listing, spatialFilter, { failOpenNoCoord: true }),
    );
    return {
      totalNumber: filteredRows.length,
      page: safePage,
      result: attachTravelTimes(filteredRows.slice(offset, offset + safePageSize).map(parseListingStatus)),
    };
  }
  ...
  // fetch page
  const rows = SqliteConnection.query(`${selectRowsSql} LIMIT @limit OFFSET @offset`, params);
