...
export const queryListings = ({
  ...
  // Spatial branch: SQLite cannot do point-in-polygon, so when a job polygon is set
  // we fetch the full WHERE/ORDER-filtered set, apply the shared spatial predicate in
  // JS (fail-open for coordinateless listings), then paginate in JS so totalNumber and
  // pages reflect the spatially filtered result. Callers that pass no spatialFilter
  // (all UI routes) fall through to the unchanged SQL LIMIT/OFFSET path below.
  if (polygonFeatures.length > 0) {
    const spatialParams = { ...params };
    delete spatialParams.limit;
    delete spatialParams.offset;
    const allRows = SqliteConnection.query(
      `SELECT l.*,
              j.name                                        AS job_name,
    ...
    const filtered = allRows.filter((l) => isListingInSpatialFilter(l, spatialFilter, { failOpenNoCoord: true }));
    const totalNumber = filtered.length;
    const pageRows = filtered.slice(offset, offset + safePageSize);
    return { totalNumber, page: safePage, result: pageRows.map(parseListingStatus) };
  }
