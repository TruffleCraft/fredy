 ...
 */
function registerAttempt(attempts, key, max) {
  const now = Date.now();
  for (const [existing, record] of attempts) {
    if (now - record.firstAttempt > LOGIN_WINDOW_MS) attempts.delete(existing);
  }
...
function isRateLimited(ip, username) {
  const overIpLimit = registerAttempt(loginAttempts, `ip:${ip}`, MAX_LOGIN_ATTEMPTS);
  const overAccountLimit =
    typeof username === 'string' && username.length > 0
      ? registerAttempt(loginAttempts, `user:${username.toLowerCase()}`, MAX_ACCOUNT_ATTEMPTS)
      : false;
  return overIpLimit || overAccountLimit;
}

function isOidcRateLimited(ip) {
  return registerAttempt(oidcAttempts, `ip:${ip}`, MAX_OIDC_ATTEMPTS);
}
