...
const oidcTrustProxy = process.env.FREDY_TRUST_PROXY === 'true';
const oidcSessionCookieSecure = process.env.FREDY_SESSION_COOKIE_SECURE === 'true';
if (oidcConfig.enabled && (!oidcTrustProxy || !oidcSessionCookieSecure)) {
...
const fastify = Fastify({
  ...
  trustProxy: oidcConfig.enabled ? oidcTrustProxy : (settings.trustProxy ?? true),
});
...
await fastify.register(fastifySession, {
  ...
  cookie: {
    ...
    httpOnly: true,
    secure: oidcConfig.enabled
      ? oidcSessionCookieSecure
      : typeof settings.baseUrl === 'string' && settings.baseUrl.startsWith('https://'),
    sameSite: 'lax',
  },
...
fastify.register(loginPlugin, {
  prefix: '/api/login',
  sessionCookieSecure: oidcSessionCookieSecure,
});
