...
import { getOidcConfig } from '../services/security/oidc.js';

const settings = await getSettings();
const PORT = settings.port || 9998;
const sessionSecret = await getOrCreateSessionSecret();
const sessionTtlHours = Number(settings.sessionTTL || 2);
if (!Number.isFinite(sessionTtlHours) || sessionTtlHours <= 0) {
  throw new Error('sessionTTL must be a positive number of hours.');
}
const SESSION_MAX_AGE = sessionTtlHours * 60 * 60 * 1000;
if (!Number.isFinite(SESSION_MAX_AGE) || Number.isNaN(new Date(Date.now() + SESSION_MAX_AGE).getTime())) {
  throw new Error('sessionTTL is too large to represent as a cookie expiration.');
}
const TRUST_PROXY = process.env.FREDY_TRUST_PROXY === 'true';
const SESSION_COOKIE_SECURE = process.env.FREDY_SESSION_COOKIE_SECURE === 'true';
const oidcConfig = getOidcConfig();
if (oidcConfig.enabled && (!TRUST_PROXY || !SESSION_COOKIE_SECURE)) {
  throw new Error('OIDC requires FREDY_TRUST_PROXY=true and FREDY_SESSION_COOKIE_SECURE=true.');
}
