...
const LEGACY_SESSION_MAX_AGE = 2 * 60 * 60 * 1000;
...
export function isUnauthorized(request) {
  ...
  if (typeof expiresAt === 'number' && Date.now() >= expiresAt) return true;
  if (typeof expiresAt !== 'number' && Date.now() - (request.session.createdAt || 0) > LEGACY_SESSION_MAX_AGE) {
    return true;
  }
  return userStorage.getUser(request.session.currentUser) == null;
}
