...
import { isWriteEnabled, registerWriteTools } from './mcpWriteTools.js';
import {
  normalizeListJobs,
  normalizeGetJob,
...
export function createMcpServer() {
  const server = new McpServer(
    ...
    },
    {
      ...
      instructions:
        ...
        (isWriteEnabled()
          ? 'Write tools are enabled: use set_listing_status to record applied/rejected/accepted, ' +
            'set_listing_notes to attach a note, and set_listing_watch to manage the watch list. ' +
            'Write tools report the persisted value back - trust that, not your own assumption. '
          : '') +
  ...
  server.tool(
    ...
    async (
      ...
      // When scoped to a job, honour its drawn area (spatial_filter) so the MCP read
      // path returns the same in-area set the UI shows. queryListings applies the
      // polygon in JS (fail-open for coordinateless listings) when provided.
      const jobForArea = jobId ? getJob(jobId) : null;
      const spatialFilter = jobForArea?.spatialFilter ?? null;
  ...
  // Listing triage writes (status, notes, watch list). No-op unless
  // FREDY_MCP_WRITE_ENABLED=true, so the default surface stays read-only.
