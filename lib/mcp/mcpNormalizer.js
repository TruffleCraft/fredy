...
export function normalizeListListings(queryResult, { page, pageSize }) {
  ...
  if (listings.length > 0) {
    ...
    for (const l of listings) {
      const geoNote =
        l.latitude == null || l.longitude == null ? ' (Geo unbekannt, Bereich nicht verifizierbar)' : '';
      md += `| ${cell(l.id)} | ${cell(l.title)} | ${cell(l.address)}${geoNote} | ${cell(l.price)} | ${cell(l.size)} | ${cell(l.provider)} | ${l.is_active ? 'yes' : 'no'} | ${cell(l.status?.status)} | ${formatDate(l.created_at)} | ${cell(l.job_name)} |\n`;
    }
...
export function normalizeGetListing(listing) {
  ...
  // Notes are part of the detail view because set_listing_notes replaces rather
  // than appends: a caller that wants to add to an existing note has to read the
  // current text first, and get_listing is the only place to get it.
...
/**
 * Normalize a write-tool response.
 *
 * Write tools report the persisted state after the change, so the caller can
 * verify the outcome instead of trusting a bare acknowledgement. Values are
 * rendered as a key-value block, matching the single-item read responses.
 *
 * @param {string} tool - The tool name.
 * @param {string} summary - Human-readable description of what changed.
 * @param {Object<string, *>} fields - Persisted values to report.
 * @returns {{ content: Array }}
 */
export function normalizeWriteResult(tool, summary, fields = {}) {
  let md = `**Tool:** ${tool} | **Status:** OK\n\n`;
  md += `${summary}\n\n`;
  for (const [key, value] of Object.entries(fields)) {
    md += `- **${key}:** ${value == null || value === '' ? '–' : cell(value)}\n`;
  }
  return toMcpResponse(md);
}
