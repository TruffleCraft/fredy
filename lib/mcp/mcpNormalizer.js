...
export function normalizeListListings(queryResult, { page, pageSize }) {
  ...
  if (listings.length > 0) {
    ...
    for (const l of listings) {
      const geoNote = l.latitude == null || l.longitude == null ? ' (geo unknown; area cannot yet be verified)' : '';
      md += `| ${cell(l.id)} | ${cell(l.title)} | ${cell(l.address)}${geoNote} | ${cell(l.price)} | ${dealTypeLabel(l.dealType)} | ${cell(l.size)} | ${cell(l.provider)} | ${cell(internetCell(l))} | ${l.is_active ? 'yes' : 'no'} | ${cell(l.status?.status)} | ${formatDate(l.created_at)} | ${cell(l.job_name)} |\n`;
    }
...
/**
 * Normalize a successful state-changing MCP response.  The caller supplies
 * values read back from storage, so a client can see what actually persisted.
 *
 * @param {string} tool
...
export function normalizeWriteResult(tool, summary, fields) {
  ...
  for (const [label, value] of Object.entries(fields)) {
    md += `- **${label}:** ${value == null || value === '' ? '–' : cell(value)}\n`;
  }
  return toMcpResponse(md);
}
