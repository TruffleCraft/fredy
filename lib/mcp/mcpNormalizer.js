...
export function normalizeListListings(queryResult, { page, pageSize }) {
  ...
  if (listings.length > 0) {
    ...
    for (const l of listings) {
      ...
      md += `| ${cell(l.id)} | ${cell(l.title)} | ${cell(l.address)}${geoNote} | ${cell(l.price)} | ${dealTypeLabel(l.dealType)} | ${cell(l.size)} | ${cell(l.provider)} | ${l.is_active ? 'yes' : 'no'} | ${cell(l.status?.status)} | ${formatDate(l.created_at)} | ${cell(l.job_name)} |\n`;
    }
